...
import sendToken from "../utils/createTokenCookies.js";
...
router.post("/register", async (req, res) => {
    try {
        ...
        if (registeredUser) return res.status(500).json(`This email ${req.body.email} is already registered`);
        ...
        //verification email
        const activationToken = jwt.sign(newUser, process.env.JWT_EMAIL_VERIFICATION_KEY, { expiresIn: "5m" });
        const activationURL = `http://localhost:3000/activation/${activationToken}`;
        try {
            await sendMail({
                email: newUser.email,
                subject: "Activate your new account",
                message: `Hello ${newUser.name}, Please click the following link to activate your new account: ${activationURL}`
            })
            res.status(201).json({
                success: true,
...
//Activate and create the new user
router.post("/activation", async(req,res,next)=>{
    
    try{
        const received_activationToken=req.body.activationToken;
        const verifiedUser=jwt.verify(received_activationToken,process.env.JWT_EMAIL_VERIFICATION_KEY);
        if(!verifiedUser){
...
router.post("/login", async (req, res) => {
    try {
        ...
        if (!getUser) return res.status(404).json("The username " + req.body.email + " is not found");
...
router.get("/stats/users-by-month", verifyAdmin, async (req, res) => {
    ...
    try {
        const groupUsers = await User.aggregate([
            ...
            {
                $group: {
                    _id: { $month: "$createdAt" }, //grouped by $month [month(createdAt)] also can be $year or $dayOfMonth or ... 
